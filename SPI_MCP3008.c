...
void main(void) 
{
	...
	*/
    ...
    while(1)
	{
		...
		t = 100.0*( ( (GetADC(0)*VREF)/1023.0) - 2.73);
		printf("%f", t);
		printf("\n"); // Carriage return only.
		
		// TOO HOT -- B6 = P15, B5 = P14, B2 = P6
		if (t > 26) {
			LATBbits.LATB6 = 1; // RED LED 
			LATBbits.LATB5 = 0; // YELLOW LED	
			LATBbits.LATB2 = 0; // BLUE LED		
		}
		// TOO COLD
		else if (t < 24) {
			LATBbits.LATB6 = 0; // RED LED
			LATBbits.LATB5 = 0; // YELLOW LED	
			LATBbits.LATB2 = 1; // BLUE LED		
		}
		// ROOM TEMP
		else {
			LATBbits.LATB6 = 0; // RED LED
			LATBbits.LATB5 = 1; // YELLOW LED	
			LATBbits.LATB2 = 0; // BLUE LED		
		}
